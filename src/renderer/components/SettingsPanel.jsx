...
const SettingsPanel = ({ isOpen, onClose, onApply }) => {
  ...
  return (
    <div className="settings-overlay" onMouseDown={handleRequestClose}>
      <div className="settings-modal" onMouseDown={e => e.stopPropagation()} style={{ display: 'flex', flexDirection: 'row', minHeight: 400 }}>
        <aside style={{ borderRight: '1px solid var(--border)', background: 'var(--panel)', padding: 12, display: 'flex', flexDirection: 'column', gap: 6, minWidth: 140 }}>
          {categories.map((cat) => (
            <button key={cat.id} onClick={() => setTab(cat.id)}
              style={{
                ...
                background: tab === cat.id ? 'color-mix(in srgb, var(--accent) 10%, transparent)' : 'transparent',
                color: 'var(--fg)',
                cursor: 'pointer',
                fontWeight: tab === cat.id ? 600 : 400
              }}
            >{cat.label}</button>
          ))}
        </aside>
        <main className="settings-body" style={{ flex: 1, overflowY: 'auto', background: 'var(--bg)' }}>
          <div className="settings-header" style={{ border: 'none', padding: 0, marginBottom: 12 }}>
            ...
            <>
                ...
                <>
                  ...
                  <div className="setting-item">
                    <label className="setting-check">
                      <input type="checkbox" checked={enableOnlineSuggestions} onChange={e => setEnableOnlineSuggestions(e.target.checked)} />
              ...
              {tab === 'privacy' && (
                <>
                  <div className="setting-item">
                    <label className="setting-check">
                      <input type="checkbox" checked={enableAdBlocker} onChange={e => setEnableAdBlocker(e.target.checked)} />
                      Enable ad & tracker blocking (beta)
                    </label>
                  </div>
                  <div className="setting-item">
                    <label className="setting-check">
                      <input type="checkbox" checked={showBookmarksBarDefault} onChange={e => setShowBookmarksBarDefault(e.target.checked)} />
              ...
              {tab === 'downloads' && (
                <>
                  <div className="setting-item">
                    <div className="setting-label">Clear Downloads</div>
                    <button className="btn secondary" onClick={() => { window.electronAPI?.clearDownloads?.(); }}>Clear All</button>
                  </div>
                </>
              )}
            </>
